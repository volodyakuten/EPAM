...
function treeMenu(data, options) {
  ...
  function createElementsTreeBydata(data) {
      ...
      if (elem.title) {
        ...
        if (elem.folder) {
          if (elem.children && elem.children.length > 0) {
            const children = createElementsTreeBydata(elem.children);
  ...
  function createTreeMenu() {
    const tree = createElementsTreeBydata(data);
